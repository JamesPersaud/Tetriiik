...
{	
	...
	int randi = GetRandomInt(0,(collectionsize-1));
...
{
	this->CurrentShape = &this->GetRandomShapeFromCollection();
...
void D3DTetrisBoard::CleanLines(int ticknow)
{
	...
	for(int y =0;y<20;y++)
	{
		...
		if(deadcount == 10)
		{
            ...
            for (int x = 0; x < 10; x++)
            {                        
                currentFragment = &D3DTetrisFragment(x, y, 0, GetRandomFloat(-50, 50), GetRandomFloat(-100, -15), GetRandomFloat(-30, 30), 0, 0, 0, 30, Blocks[(y * 10) + x].GetColour(),ticknow);                       
                Fragments.push_back(*currentFragment);
                currentFragment = &D3DTetrisFragment(x + 0.5f, y, 0, GetRandomFloat(-50, 50), GetRandomFloat(-100, -15), GetRandomFloat(-30, 30), 0, 0, 0, 30, Blocks[(y * 10) + x].GetColour(),ticknow);                       
                Fragments.push_back(*currentFragment);
                currentFragment = &D3DTetrisFragment(x, y + 0.5f, 0, GetRandomFloat(-50, 50), GetRandomFloat(-100, -15), GetRandomFloat(-30, 30), 0, 0, 0, 30, Blocks[(y * 10) + x].GetColour(),ticknow);                                            
                Fragments.push_back(*currentFragment);
                currentFragment = &D3DTetrisFragment(x + 0.5f, y + 0.5f, 0, GetRandomFloat(-50, 50), GetRandomFloat(-100, -15), GetRandomFloat(-30, 30), 0, 0, 0, 30, Blocks[(y * 10) + x].GetColour(),ticknow);                                           
                Fragments.push_back(*currentFragment); 
            }
